/**
 * Chen Learning Companion
 * 
 * Learning and professional development management dashboard for Chen Family
 * Matches the AldrLearning.js structure from Peter Murphy demo
 */
...
const ChenLearning = () => {
  ...
  const getCategoryIcon = (category) => {
    const iconMap = {
      language_learning: 'fa-globe',
      health_education: 'fa-heartbeat',
      formal_education: 'fa-graduation-cap',
      professional_certification: 'fa-certificate',
      professional_development: 'fa-briefcase',
      soft_skills: 'fa-users',
      technical_skills: 'fa-laptop-code',
      online_courses: 'fa-computer',
      other: 'fa-file-alt'
    };
  ...
  const getRecords = () => {
    if (familyMode) {
      ...
      const sarahRecords = chenFamilyData.sarah.learning.map(record => ({ ...record, owner: 'Sarah Chen' }));
      const davidRecords = chenFamilyData.david.learning.map(record => ({ ...record, owner: 'David Chen' }));
      const sharedRecords = chenFamilyData.familyMode.sharedDocuments
        .filter(doc => doc.vault === 'learning')
        .map(doc => ({ ...doc, owner: 'Shared' }));
      
      return [...sarahRecords, ...davidRecords, ...sharedRecords]
        .sort((a, b) => new Date(b.completed || b.graduationDate || b.startDate || b.date || '2024-01-01') - new Date(a.completed || a.graduationDate || a.startDate || a.date || '2024-01-01'));
    } else {
      // Individual mode: show selected person's records
      return selectedPerson === 'sarah' 
        ? chenFamilyData.sarah.learning 
        : chenFamilyData.david.learning;
    }
  };
  ...
  // Use Chen family learning records
  const actualRecords = getRecords();

  // Learning reminders (expiring certifications)
  const learningReminders = actualRecords
    .filter(record => record.expiryDate)
    .map(record => ({
      id: record.id,
      title: record.title,
      dueDate: record.expiryDate,
      lastDate: record.startDate || record.completed || record.graduationDate,
  ...
  const processRecords = () => {
    ...
    const recordsCopy = [...actualRecords].sort((a, b) => new Date(b.completed || b.graduationDate || b.startDate || b.date || '2024-01-01') - new Date(a.completed || a.graduationDate || a.startDate || a.date || '2024-01-01'));
    ...
    const activeCertifications = recordsCopy
      .filter(record => 
        ...
        (record.category === 'professional_certification' || record.category === 'language_learning')
      )
      .slice(0, 3);
  ...
  return (
    <div className="dashboard-container">
      {/* Header matching AldrLearning style */}
      <header className="dashboard-header">
        ...
        <div className="header-center flex flex-col items-center justify-center">
          <div className="flex items-center">
            ...
            <h1 className="text-white text-4xl" style={{ fontFamily: 'Lora, serif', fontWeight: '500' }}>
              {getPersonDisplayName()} - Aldr Learning
            </h1>
          </div>
        ...
        <div className="header-actions">
          <button className="dashboard-button white" onClick={() => alert('Demo mode: Add learning credential functionality coming in production version')}>
            <i className="fas fa-plus"></i>
            <span className="hidden sm:inline">Add Credential</span>
          </button>
          <button className="dashboard-button white" onClick={() => alert('Demo mode: Learning sharing functionality coming in production version')}>
          ...
          <div className="divide-y">
            ...
            {learningReminders.length === 0 && (
              <div className="py-4 text-center text-aldr-gray">
                No upcoming learning reminders
              </div>
            )}
          </div>
        </div>
              ...
              <div key={cert.id} className="card hover:shadow-lg transition-all duration-300">
                <div className="flex items-center mb-4">
                  ...
                  <div className="flex-grow">
                    ...
                    <span className={`px-2 py-1 rounded-pill text-xs ${getStatusColor(cert.status || 'active')}`}>
                      {cert.status || 'Active'}
                    </span>
                    {familyMode && cert.owner && (
                ...
                <p className="text-sm text-aldr-gray mb-2">{cert.provider || cert.institution}</p>
                <p className="text-sm text-aldr-gray mb-2">Issued: {formatDate(cert.completed || cert.graduationDate || cert.startDate)}</p>
                {cert.expiryDate && (
                  <p className="text-sm text-aldr-gray mb-4">
                    Expires: {formatDate(cert.expiryDate)}
                    <span className={`ml-2 px-2 py-1 rounded-pill text-xs ${
                      getDaysUntilExpiration(cert.expiryDate) < 90 ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'
                    }`}>
                      {getDaysUntilExpiration(cert.expiryDate)} days
                    </span>
                  </p>
                )}
              ...
              <div key={record.id} className="card hover:shadow-lg transition-all duration-300">
                <div className="flex items-center mb-4">
                  ...
                  <div className="flex-grow">
                    ...
                    <span className={`px-2 py-1 rounded-pill text-xs ${getStatusColor(record.status || 'completed')}`}>
                      {record.status || 'Completed'}
                    </span>
                ...
                <p className="text-sm text-aldr-gray mb-2">{record.provider || record.institution}</p>
                <p className="text-sm text-aldr-gray mb-4">{formatDate(record.completed || record.graduationDate || record.startDate)}</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {records.slice(0, 2).map(record => (
                    <div key={record.id} className="border rounded-lg p-4 hover:shadow-md transition">
                      ...
                      <p className="text-sm text-aldr-gray mb-2">{record.provider || record.institution}</p>
                      <p className="text-sm text-aldr-gray mb-2">{formatDate(record.completed || record.graduationDate || record.startDate)}</p>
                      {record.expiryDate && (
        ...
        <div className="card">
          <div className="divide-y">
            <div className="py-3 flex items-center">
              ...
              <div className="flex-grow">
                <p className="font-medium text-aldr-dark">Viewed {getPersonDisplayName()} Learning Records</p>
            ...
            <div className="py-3 flex items-center">
              ...
              <div className="flex-grow">
                <p className="font-medium text-aldr-dark">Accessed Chen Family Learning Vault</p>
                <p className="text-sm text-aldr-gray">Today</p>
              </div>
              <button className="bg-aldr-teal hover:bg-aldr-purple text-white py-1 px-3 rounded-pill transition-colors text-sm">
                Open
