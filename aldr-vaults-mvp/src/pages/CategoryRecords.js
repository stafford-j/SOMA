...
const CategoryRecords = () => {
  ...
  // Filter for specific category records
  const categoryRecords = sampleRecords.filter(record => record.specialty === category);
  ...
  const sortedRecords = [...categoryRecords].sort((a, b) => new Date(b.date) - new Date(a.date));

  const categoryDisplayName = formatSpecialtyName(category);
      ...
      <div className="max-w-6xl mx-auto">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl md:text-4xl font-bold heading-gradient mb-4">
                {categoryDisplayName} Records
              </h1>
              <p className="text-aldr-gray">
                All your {categoryDisplayName.toLowerCase()} records ({sortedRecords.length} total)
              </p>
            </div>
            <Link
              to="/"
        ...
        {sortedRecords.length === 0 && (
          <div className="text-center py-8">
            <p className="text-aldr-gray">No {categoryDisplayName.toLowerCase()} records found</p>
          </div>
        )}
      </div>
    </div>
