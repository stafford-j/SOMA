        ...
        let pod_type = self.client.analyze_address(address, false).await.unwrap_or_else(|e| -> Analysis {
            match e {
                AnalysisError::FailedGet => {
                    ...
                    // Handle key lookup failures gracefully instead of panicking
                    if self.data_store.address_is_pointer(address).unwrap_or(false) {
                        match self.key_store.get_pointer_key(address.to_string()) {
                            Ok(pointer_key) => {
                                match SecretKey::from_hex(pointer_key.trim()) {
                                    Ok(secret_key) => {
                                        Analysis::Pointer(Pointer::new(
                                            &secret_key,
                                            0,
                                            PointerTarget::ScratchpadAddress(ScratchpadAddress::new(secret_key.public_key())),
                                        ))
                                    },
                                    Err(e) => {
                                        error!("Failed to create SecretKey from pointer key for address {}: {}", address, e);
                                        Analysis::Chunk(Chunk::new(Bytes::new()))
                                    }
                                }
                            },
                            Err(e) => {
                                error!("Pointer key not found for address {}: {}", address, e);
                                Analysis::Chunk(Chunk::new(Bytes::new()))
                            }
                        }
                    } else if self.data_store.address_is_scratchpad(address).unwrap_or(false) {
                        match self.key_store.get_scratchpad_key(address.to_string()) {
                            Ok(scratchpad_key) => {
                                match SecretKey::from_hex(scratchpad_key.trim()) {
                                    Ok(secret_key) => {
                                    ...
                                    Err(e) => {
                                        error!("Failed to create SecretKey from scratchpad key for address {}: {}", address, e);
                                        Analysis::Chunk(Chunk::new(Bytes::new()))
                                    }
                                }
                            },
                            Err(e) => {
                                error!("Scratchpad key not found for address {}: {}", address, e);
                                Analysis::Chunk(Chunk::new(Bytes::new()))
                            }
                        }
                    } else {
