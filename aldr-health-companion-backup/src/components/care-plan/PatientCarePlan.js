 ...
 * SOMA Colleague and SOMA Companion.
 * 
 * Features:
...
const PatientCarePlan = () => {
              ...
              <svg viewBox="0 0 36 36" className="progress-ring">
                ...
                <path className="progress-ring-fill"
                  ...
                  d="M18 2.0845
                    ...
                    a 15.9155 15.9155 0 0 1 0 -31.831"
                />
                <text x="18" y="20.35" className="progress-text">{calculateOverallProgress()}%</text>
              </svg>
            </div>
              ...
              {todayTasks.map(task => (
                <div key={task.id} className={`task-item ${task.completionStatus ? 'completed' : ''}`}>
                  ...
                  <div className="task-content">
                    ...
                    {isOpinionMode && task.id === 'task-001' && (
                      <div className="task-insight">
                        <i className="fas fa-lightbulb"></i>
                        <p>Consistent morning glucose checks help identify patterns. Try to measure at the same time each day.</p>
                      </div>
                    )}
                  </div>
          ...
          <div className="measurements-grid">
            <div className="measurement-card">
              ...
              {isOpinionMode && (
                <div className="measurement-insight">
                  <i className="fas fa-lightbulb"></i>
                  <p>{localPlan.insights.goals.bloodGlucose.summary}</p>
                </div>
              )}
            ...
            <div className="measurement-card">
              ...
              {isOpinionMode && (
                <div className="measurement-insight">
                  <i className="fas fa-lightbulb"></i>
                  <p>{localPlan.insights.goals.weight.summary}</p>
                </div>
              )}
              <button className="log-new-btn">
                <i className="fas fa-plus"></i> Log New Reading
          ...
          <div className="goals-list">
            {localPlan.goals.map(goal => (
              <div key={goal.id} className="goal-card">
                ...
                {isOpinionMode && goal.id === 'goal-001' && (
                  <div className="goal-insight">
                    <i className="fas fa-lightbulb"></i>
                    <p>{localPlan.insights.goals.bloodGlucose.summary}</p>
                  </div>
                )}
                {isOpinionMode && goal.id === 'goal-002' && (
                  <div className="goal-insight">
                    <i className="fas fa-lightbulb"></i>
                    <p>{localPlan.insights.goals.weight.summary}</p>
                  </div>
          ...
          <div className="wellbeing-section">
            ...
            <div className="wellbeing-card">
              <div className="wellbeing-header">
                ...
                <h4>Managing Stress with Diabetes</h4>
              </div>
              <p className="wellbeing-text">{localPlan.insights.wellbeing.summary}</p>
              <ul className="wellbeing-recommendations">
                {localPlan.insights.wellbeing.recommendations.map((rec, index) => (
                  <li key={index}>{rec}</li>
                ))}
              </ul>
              <div className="insight-sources">
                <p className="sources-label">Sources:</p>
                <p className="sources-text">{localPlan.insights.wellbeing.sources.join(', ')}</p>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="travel-card">
            ...
            {isOpinionMode && (
              <div className="travel-insights">
                ...
                <p>{localPlan.insights.travel.summary}</p>
                <ul className="insight-recommendations">
                  {localPlan.insights.travel.recommendations.map((rec, index) => (
                    <li key={index}>{rec}</li>
                  ))}
                </ul>
                <div className="insight-sources">
                  <p className="sources-label">Sources:</p>
                  <p className="sources-text">{localPlan.insights.travel.sources.join(', ')}</p>
                </div>
              </div>
            )}
  ...
  return (
    <div className={`patient-care-plan ${isOpinionMode ? 'opinion-mode' : 'data-mode'}`}>
      <div className="care-plan-header">
        ...
        <div className="header-right">
          <div className="mode-toggle-container">
            <button 
              className={`mode-toggle ${!isOpinionMode ? 'active' : ''}`}
              onClick={() => setIsOpinionMode(false)}
            >
